...
export default function MediaDetailPage({ type }: Props) {
  ...
  const { user } = useAuth();
  const { isNsfw, hasNsfwKeyword, addNsfwIds, disableBlur } = useNsfwFilter();
  ...
  const mediaKeywordIds = (media.keywords?.keywords ?? media.keywords?.results ?? []).map((k) => k.id);
  const nsfw = !revealed && (isNsfw(media.id) || hasNsfwKeyword(mediaKeywordIds));
      ...
      {showNsfwModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowNsfwModal(false)}>
          <div className="bg-ndp-bg rounded-2xl w-full max-w-sm mx-4 shadow-2xl shadow-black/60 p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-sm text-ndp-text-muted mb-6">{t('nsfw.modal.description')}</p>
            <div className="flex flex-col gap-2">
              ...
              <button
                ...
                className="w-full px-4 py-2.5 bg-red-600/10 hover:bg-red-600/20 text-red-400 text-sm font-medium rounded-xl transition-colors"
              >
