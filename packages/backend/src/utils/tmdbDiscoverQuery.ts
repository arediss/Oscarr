/** TMDB discover query → URLSearchParams. Mirrored in frontend/src/utils/tmdbDiscoverQuery.ts
...
/** Return [gteField, lteField] for the given media type. */
...
/** Resolve a relative release window (last_30d / last_90d / last_6m / last_1y) to a
 *  `{ gte, lte }` ISO-date pair. `lte` is always "today" to cap the window at now. */
...
/** Build the URLSearchParams for TMDB's /discover endpoint from a query object. */
