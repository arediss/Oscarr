...
const embyAuth: AuthProvider = {
    ...
    app.post('/emby/login', {
      ...
      try {
        ...
        const result = await helpers.findOrCreateUser({
          ...
          email: `${auth.user.name.toLowerCase().replaceAll(/[^a-z0-9]/g, '')}@emby.local`,
  ...
  async linkAccountByCredentials(username, password, userId) {
    ...
    await prisma.userProvider.upsert({
      ...
      create: { userId, provider: 'emby', providerId: auth.userId, providerToken: auth.token, providerUsername: auth.user.name, providerAvatar: avatar ?? null },
    });
  ...
  async importUsers(_adminUserId, filter) {
    ...
    for (const user of users) {
      ...
      const email = `${user.name.toLowerCase().replaceAll(/[^a-z0-9]/g, '')}@emby.local`;
